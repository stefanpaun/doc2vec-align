...
class VicinityDrivenParagraphAligner(ParagraphAligner):
	...
	* *Parameters*:
	...
	def __init__(self, similarity_model, acceptable_similarity):
	...
	def alignParagraphsFromDocuments(self, p1s=[], p2s=[]):
		...
		* *Output*:
		...
		return alignment_path, aligned_paragraphs, paragraph_similarities
	...
	* *Parameters*:
	...
	def getSentenceAlignmentPath(self, p1, p2, sentence_similarities, sentence_indexes):
		...
		* *Output*:
		...
		#Start vicinity-driven path search:
		#matrix = sentence_similarities#self.getProbabilityMatrix(p1, p2, sentence_similarities, sentence_indexes)
		
		#Start search for alignment path:
		path = []
		cbuffer = ''
		sbuffer = ''
		starting_point = self.findStartingPoint(sentence_similarities, p1, p2, [-1,-1])
		...
		while currXY[0]<len(p1)-1 and currXY[1]<len(p2)-1:
			bestNextXY, bestNextXYProb = self.getBestNextHypothesis(sentence_similarities, p1, p2, cbuffer, sbuffer, currXY)
			...
			#Check to see if downards is best:
				...
				while anchor<len(p1) and currsim>sentence_similarities[anchor][bestNextXY[1]+1] and currsim>prevsim-self.similarity_slack:
					anchor += 1
					if anchor<len(p1):
						prevsim = currsim
						currsim = self.similarity_model.getTextSimilarity(cbuffer+' '+p1[anchor], sbuffer)
						if currsim>prevsim-self.similarity_slack and currsim>sentence_similarities[anchor][bestNextXY[1]+1]:
							cbuffer += ' ' + p1[anchor]
				...
				if anchor<len(p1):
					currXY = self.findStartingPoint(sentence_similarities, p1, p2, [anchor-1, bestNextXY[1]])
			...
			#Check to see if rightwards is best:
				...
				currsim = bestNextXYProb
				while anchor<len(p2) and currsim>sentence_similarities[bestNextXY[0]+1][anchor] and currsim>prevsim-self.similarity_slack:
					anchor += 1
					if anchor<len(p2):
						prevsim = currsim
						currsim = self.similarity_model.getTextSimilarity(cbuffer, sbuffer+' '+p2[anchor])
						if currsim>prevsim-self.similarity_slack and currsim>sentence_similarities[bestNextXY[0]+1][anchor]:
							sbuffer += ' ' + p2[anchor]
				...
				#If edge is not reached, find new starting point to continue the alignment search:
				if anchor<len(p2):
					currXY = self.findStartingPoint(sentence_similarities, p1, p2, [bestNextXY[0], anchor-1])
					if currXY[0]<len(p1) and currXY[1]<len(p2):
						cbuffer = p1[currXY[0]]
						final_cbuffer = [currXY[0]]
						sbuffer = p2[currXY[1]]
...
class VicinityDrivenParagraphAlignerAlternate(ParagraphAligner):
	"""
	Implements the vicinity-driven paragraph alignment algorithm proposed in:
	...
	* *Parameters*:
	...
	def __init__(self, similarity_model, certain_threshold = 0.85, hard_threshold = 0.6, soft_threshold = 0.3):
		self.vicinity = set([(1,1),(2,1),(1,2)]) 
	...
	def alignParagraphsFromDocuments(self, p1s=[], p2s=[]):
		...
		* *Output*:
		...
		#Return alignment path:
		return alignment_path, aligned_paragraphs, self.paragraph_similarities
		...
		while currXY[0]<sizep1-1 or currXY[1]<sizep2-1:
			...
			if nextXYsim >= self.hard_threshold:
				for element in nextXY:
					path.append(element)
			currXY = nextXY[-1]
		# 	print("Last alignment considered", currXY)
		# 	print("\n")
	...
	def getNextAlignment(self, currXY):
		...
		* *Output*:
		...
		candXY_sim = cands[(candXY[0], candXY[1])]
		
		if candXY_sim >= self.soft_threshold:
			path, candXY_sim = self.checkCandidate(candXY, candXY_sim)
			return path, candXY_sim
		else:
			#all = [cands[c] for c in cands]
			#If not, get a next synchronizer outside the reachable vicinity
			if np.max(cands.values()) < self.soft_threshold:
				finalNextXY, final_sim = self.getNextSynchronizer(candXY, candXY_sim)
				#print("V3", finalNextXY, final_sim)
				return finalNextXY, final_sim
			else:
				new_cands = {}
				for element in cands.keys():
					if (cands[element] >= self.soft_threshold): 
						path, candXY_sim = self.checkCandidate(element, cands[element])
						new_cands[candXY_sim] = path
				max_sim = max(new_cands.keys())
				#print("V2", new_cands[max_sim], max_sim)
				return new_cands[max_sim], max_sim
				

	def checkCandidate(self, candXY, candXY_sim):
		if candXY_sim > self.certain_threshold:
			return [candXY], candXY_sim
		ok = True
		init_sim = candXY_sim
		path = [candXY]
		lastXY = candXY
		par_orig = str(self.paragraphs[0][lastXY[0]])
		par_simp = str(self.paragraphs[1][lastXY[1]])
		# print("-----Checking Candidate-----")
		# print(candXY_sim)
		while ok:
			if (lastXY[0] < len(self.paragraphs[0])-1 and lastXY[1]+1 < len(self.paragraphs[1])):
				potential_candidates = dict()
				sim_om1 = self.getNewParagraphSimilarity(par_orig, par_simp+str(self.paragraphs[1][lastXY[1]+1]))
				sim_mo1 = self.getNewParagraphSimilarity(par_orig+str(self.paragraphs[0][lastXY[0]+1]), par_simp)
				# print("Last, om, mo, ", candXY_sim, sim_om1, sim_mo1)
				...
				ok = False
		# print("Path from checkingCandidate: ", path, candXY_sim)
		return path, candXY_sim
	...
	def getNextSynchronizer(self, currXY, candXY_sim):
		...
		* *Output*:	
		...
		for i in range(orig[0], last[0]):
			for j in range(orig[1], last[1]):
				if self.paragraph_similarities[i][j]>=self.soft_threshold:
					path, candXY_sim = self.checkCandidate((i,j), self.paragraph_similarities[i][j])
					if candXY_sim > self.hard_threshold:
						# print(path, candXY_sim)
						return path, candXY_sim	
